...
func main() {
	ctx, cancel := context.WithTimeout(context.TODO(), 0*time.Minute)
	defer cancel()
...
func verifyFunction(ctx context.Context, w *sync.WaitGroup) {
	defer w.Done()
	...
	for i := 0; i < 3; i++ {
		...
		fmt.Println("开始验证服务")
		time.Sleep(100 * time.Millisecond) //用来代替验证环节，比如：服务调用、服务模拟等
		if i <= 1 {
